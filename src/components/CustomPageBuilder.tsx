...
import {
  BLOCK_KIND_LABELS,
  defaultBlockPalette,
} from "@/components/customLayoutShared";
...
const BUILDER_APPEARANCE_OPTIONS: AppearancePreset[] = [
  "soft",
  "flat",
  "glass",
  "custom",
...
export default function CustomPageBuilder({
  ...
  const hasBuilderPaletteOverrides =
    Object.keys(builderPaletteOverrides).length > 0;
  const isCanvasPrefsDirty =
    builderAppearance !== "soft" || !showBuilderGuides || showBuilderSummary;
    ...
    () => [
      {
        ...
        title: "Master dimmer",
        description:
          "Jednoduch├Ż slidery nebo kole─Źko pro ovl├íd├ín├ş celkov├ę intenzity se zobrazen├şm procent.",
        configHint:
          'Napoj├ş┼í na useKV("master-dimmer") a helper setMasterDimmerScale ÔÇô viz LiveControlView.',
      },
      {
        kind: "scene-button",
        title: "Sc├ęnick├ę tla─Ź├ştko",
        description:
          "Spust├ş/preview konkr├ętn├ş sc├ęnu jedn├şm klikem, ide├íln├ş pro dotykov├ę ovl├íd├ín├ş.",
      ...
      },
      {
        ...
        description:
          "P┼Ö├şm├ę ┼Ö├şzen├ş konkr├ętn├şho DMX kan├ílu (nap┼Ö. dimmer, barva, iris).",
        configHint:
          "Vyber fixture + channelId, hodnoty se zapisuj├ş p┼Öes setFixtures Ôçĺ persistShowSnapshot.",
      },
      {
        kind: "motor-pad",
        title: "Motorick├Ż pad",
        description:
          "2D pad/joystick pro sm─Ťrov├ín├ş stepper motoru nebo pan/tilt pohybu.",
      ...
      },
      {
        ...
        title: "Servo knob",
        description:
          "Mal├Ż kruhov├Ż ovlada─Ź pro servo ├║hel, s mo┼żnost├ş uk├ízat c├şlovou hodnotu.",
      ...
      },
      {
        ...
        description:
          "Statick├Ż blok s instrukcemi, QR k├│dem nebo rozcestn├şkem.",
        configHint:
          "Ulo┼żen├Ż text se bude renderovat p┼Öes markdown renderer ÔÇô vhodn├ę pro ops handover.",
      },
    ],
  ...
  const filteredBlocks = useMemo(() => {
    ...
    return availableBlocks.filter((block) => {
      const haystack =
        `${block.kind} ${block.title} ${block.description} ${block.configHint}`.toLowerCase();
      ...
      reader.onload = () => {
        try {
          ...
          applyLayoutUpdate(() => ({
            ...
            grid: {
              columns: clampValue(
                typeof parsed.grid?.columns === "number"
                  ? parsed.grid.columns
                  : layoutPreview.grid?.columns ?? 12,
                4,
                24
              ),
              rowHeight:
    ...
    switch (kind) {
      ...
      case "fixture-slider": {
        ...
        return {
          ...
          kind,
          title: firstFixture
            ? `${firstFixture.name} ${firstChannel?.name ?? ""}`.trim()
            : "Kan├íl",
          size: "md",
          position: basePosition,
  ...
  const handleRemoveBlock = useCallback(
    (blockId: string) => {
      applyLayoutUpdate((current) => {
        ...
        const nextBlocks = current.blocks.filter(
          (block) => block.id !== blockId
        );
        const nextSelectionTarget = nextBlocks[0]?.id ?? null;
        setSelectedBlockId((currentSelected) =>
          currentSelected === blockId ? nextSelectionTarget : currentSelected
        );
  ...
  const updateBlock = (
    blockId: string,
    updater: (block: CustomBlock) => CustomBlock
  ) => {
    applyLayoutUpdate((current) => {
      if (!current) {
        return current;
  ...
  const updateLayoutGrid = (
    partial: Partial<NonNullable<CustomLayout["grid"]>>
  ) => {
    applyLayoutUpdate((current) => {
      const base = ensureLayoutBase(current);
      const nextGrid = {
    ...
    (ids: string[], key: string, delta: number, resize: boolean) => {
      ...
      applyLayoutUpdate((current) => {
        ...
        const nextBlocks = current.blocks.map((block) => {
          ...
          if (resize) {
            if (key === "ArrowRight") {
              nextPos.width = clampValue(
                nextPos.width + delta,
                1,
                Math.max(1, columns - nextPos.col)
              );
            ...
            if (key === "ArrowRight") {
              nextPos.col = clampValue(
                nextPos.col + delta,
                0,
                Math.max(0, columns - nextPos.width)
              );
            } else if (key === "ArrowLeft") {
    ...
    <div className="flex flex-col gap-4">
      <Card className="p-6">
        ...
        <p className="text-sm text-muted-foreground">
          Definovali jsme z├íkladn├ş typy blok┼» a layout se brzy bude ukl├ídat
          spolu s show snapshotem. Ka┼żd├Ż blok m┼»┼że m├şt vlastn├ş konfiguraci
          (fixture, sc├ęna, efekt) a renderer si zvol├ş v├Żslednou podobu.
        </p>
      </Card>
      <Card className="p-6">
        ...
        <div className="grid gap-4 md:grid-cols-3">
            ...
            <Input
              ...
              onChange={(event) =>
                updateLayoutGrid({
                  columns: Math.min(
                    24,
                    Math.max(4, Number(event.target.value) || 12)
                  ),
                })
              }
            />
          ...
          <div className="space-y-1">
            ...
            <Input
              ...
              onChange={(event) =>
                updateLayoutGrid({
                  rowHeight: Math.max(0.5, Number(event.target.value) || 1),
                })
              }
            />
          </div>
          <div className="space-y-1">
            ...
            <Input
              ...
              onChange={(event) =>
                updateLayoutGrid({
                  gap: Math.max(0, Number(event.target.value) || 1),
                })
              }
            />
          </div>
        ...
        <p className="text-xs text-muted-foreground mt-3">
          ├Üprava parametr┼» m┼Ö├ş┼żky se projev├ş jak v pl├ítn─Ť, tak v runtime
          rendereru. Hodnoty se ukl├ídaj├ş do `customLayout.grid`. Tip: dr┼ż{" "}
          <kbd>Shift</kbd> a t├íhni bloky pro p┼Öesn─Ťj┼í├ş zarovn├ín├ş
          (p┼Öipravuje se snapÔÇĹtoÔÇĹgrid).
        </p>
      </Card>
      <Card className="p-6 space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              {layoutPreview.grid?.columns ?? 12} sloupc┼» ┬Ě mezera{" "}
              {layoutPreview.grid?.gap ?? 1}
            </p>
          </div>
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button
                      ...
                      onClick={() =>
                        setModifier(
                          modifier === "duplicate" ? "none" : "duplicate"
                        )
                      }
                    >
                      Duplicita
                  ...
                  <TooltipContent>
                    Zapni pro rychl├ę klonov├ín├ş blok┼» kliknut├şm
                  </TooltipContent>
                </Tooltip>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button
                      size="sm"
                      variant={
                        modifier === "delete" ? "destructive" : "outline"
                      }
                      onClick={() =>
                        setModifier(modifier === "delete" ? "none" : "delete")
                      }
                    >
                      Maz├ín├ş
                    </Button>
                  ...
                  <TooltipContent>
                    Zapni pro rychl├ę maz├ín├ş blok┼» kliknut├şm
                  </TooltipContent>
                </Tooltip>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button
                      ...
                      onClick={() =>
                        setModifier(modifier === "multi" ? "none" : "multi")
                      }
                    >
                      Multi v├Żb─Ťr
                    </Button>
                  ...
                  <TooltipContent>
                    P┼Öid├ívej/odeb├şrej bloky do v├Żb─Ťru kliknut├şm
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
            </div>
        ...
        <p className="text-xs text-muted-foreground">
          Tip: Ctrl/Cmd + D klonuje vybran├Ż blok, Delete jej odebere. Alt +
          ┼íipky m─Ťn├ş velikost, samotn├ę ┼íipky posouvaj├ş v├Żb─Ťr (Shift =
          v─Ťt┼í├ş krok).
        </p>
      </Card>
      <Card className="p-6 space-y-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Vylad├ş p├şedvolby rendereru a pracuj s JSON snapshoty.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Button
              size="sm"
              variant="ghost"
              onClick={resetBuilderCanvasPrefs}
              disabled={!isCanvasPrefsDirty}
            >
              Reset pl├ítna
            </Button>
            <Button
              size="sm"
              variant="ghost"
              onClick={resetBuilderPalette}
              disabled={!hasBuilderPaletteOverrides}
            >
              Reset palety
            </Button>
          </div>
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <div className="flex items-center justify-between rounded-xl border bg-muted/30 p-3">
            <div className="pr-4">
              ...
              <p className="text-xs text-muted-foreground">
                Vykresl├ş horizont├ílni i vertik├ílni m┼Ö├şku pro p┼Öesn├ş
                zarovn├ín├ş.
              </p>
            </div>
            <Switch
              checked={showBuilderGuides}
              onCheckedChange={setShowBuilderGuides}
            />
          </div>
          <div className="flex items-center justify-between rounded-xl border bg-muted/30 p-3">
            <div className="pr-4">
              ...
              <p className="text-xs text-muted-foreground">
                Zobraz├ş metriky layoutu nad rendererem.
              </p>
            </div>
            <Switch
              checked={showBuilderSummary}
              onCheckedChange={setShowBuilderSummary}
            />
          </div>
        </div>
        <div>
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Gradienty pro jednotliv├ę bloky (Tailwind utility).
              </p>
            </div>
            ...
            {CUSTOM_BLOCK_KIND_ORDER.map((kind) => (
              ...
              >
                ...
                <Input
                  ...
                  onChange={(event) =>
                    setBuilderPaletteEntry(kind, event.target.value)
                  }
                  placeholder={
                    defaultBlockPalette[kind] ??
        ...
        <p className="text-xs text-muted-foreground">
          Exportuje se aktu├ílne zvolen├ş <code>customLayout</code>. Import
          p┼Öep├ş┼í layout v─Ťetn├ě gridu a ulo┼ż├ş krok do historie pro
          undo/redo.
        </p>
      </Card>
      <Card className="p-6">
        ...
        <p className="text-sm text-muted-foreground mt-6">
          Bloky p┼Öid├í┼í jedn├şm klikem a pot├ę je m┼»┼że┼í p┼Öetahovat n├ş┼że
          ÔÇô konfigurace se ukl├íd├í do{" "}
      ...
      <Card className="p-6">
        <div className="flex items-center justify-between mb-2">
          ...
          <p className="text-xs text-muted-foreground">
            {blocks.length
              ? `${blocks.length} blok┼» ┬Ě aktualizov├íno ${
                  customLayout?.updatedAt
                    ? new Date(customLayout.updatedAt).toLocaleString()
                    : "nikdy"
                }`
              : "Zat├şm ┼ż├ídn├ę bloky"}
          </p>
        </div>
        <Separator className="my-3" />
        {blocks.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            P┼Öidej prvn├ş blok v├Ż┼íe a layout se ulo┼ż├ş do show snapshotu.
          </p>
        ) : (
          <DndContext
            sensors={sensors}
                ...
                {blocks.map((block) => (
                  <SortableBlockRow
                    ...
                    onSelect={(id, mode) =>
                      handleSelectBlock(
                        id,
                        mode ?? (modifier === "multi" ? "toggle" : "replace")
                      )
                    }
      ...
      <Card className="p-6">
        ...
        {selectionCount > 1 ? (
          <p className="text-sm text-muted-foreground">
            Vybr├íno {selectionCount} blok┼». Pou┼żij bulk akce v├Ż┼íe nebo
            zmen┼íi v├Żb─Ťr, pokud chce┼í upravovat konkr├ętn├ş blok.
          </p>
        ) : !selectedBlock ? (
          <p className="text-sm text-muted-foreground">
            Vyber blok v seznamu nebo na pl├ítn─Ť pro ├║pravu.
          </p>
        ) : (
          <BlockEditor
            block={selectedBlock}
  ...
  return (
    <div
      ...
      onClick={(event) =>
        onSelect(
          block.id,
          event.metaKey || event.ctrlKey || multiEnabled ? "toggle" : "replace"
        )
      }
      className={cn(
...
function BlockEditor({
  ...
  const updateCurrent = (partial: Partial<CustomBlock>) => {
    onUpdate(
      (currentBlock) => ({ ...currentBlock, ...partial } as CustomBlock)
    );
  };

  const handlePositionChange = (
    field: keyof CustomBlockPosition,
    value: number
  ) => {
    ...
    if (field === "col") {
      ...
      next.width = clampGridValue(
        next.width,
        1,
        Math.max(1, columns - next.col)
      );
    } else if (field === "row") {
    ...
    switch (block.kind) {
          ...
          <>
            ...
            <div className="space-y-1">
              ...
              <Select
                ...
                onValueChange={(value: SceneButtonBlock["behavior"]) =>
                  updateCurrent({ behavior: value } as SceneButtonBlock)
                }
              >
                <SelectTrigger>
                  <SelectValue />
          ...
          <>
            <div className="space-y-1">
              ...
              <Select
                ...
                onValueChange={(value) =>
                  updateCurrent({
                    effectId: value === "none" ? null : value,
                  } as EffectToggleBlock)
                }
              >
                <SelectTrigger>
            ...
            <div className="space-y-1">
              ...
              <Select
                value={block.behavior}
                onValueChange={(value: EffectToggleBlock["behavior"]) =>
                  updateCurrent({ behavior: value } as EffectToggleBlock)
                }
              >
                <SelectTrigger>
                  <SelectValue />
      ...
      case "fixture-slider": {
        const selectedFixture =
          fixtures.find((fixture) => fixture.id === block.fixtureId) ??
          fixtures[0];
            ...
            <div className="grid gap-3 md:grid-cols-2">
              ...
              <div className="space-y-1">
                ...
                <Input
                  ...
                  onChange={(event) =>
                    updateCurrent({
                      max: Number(event.target.value) || 255,
                    } as FixtureSliderBlock)
                  }
                />
              </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <Switch
                ...
                onCheckedChange={(checked) =>
                  updateCurrent({ showValue: checked } as FixtureSliderBlock)
                }
              />
            </div>
          </>
        ...
        return (
          <>
            ...
            <div className="space-y-1">
              ...
              <Select
                value={block.axis}
                onValueChange={(value: MotorPadBlock["axis"]) =>
                  updateCurrent({ axis: value } as MotorPadBlock)
                }
              >
                <SelectTrigger>
                  <SelectValue />
            ...
            <div className="space-y-1">
              ...
              <Input
                ...
                onChange={(event) =>
                  updateCurrent({
                    speedScale: Number(event.target.value) || 1,
                  } as MotorPadBlock)
                }
              />
            </div>
      ...
      case "servo-knob":
        return (
          <>
            ...
            <div className="flex items-center justify-between">
              <Label htmlFor="servo-show-target">
                Zobrazit c├şlovou hodnotu
              </Label>
              <Switch
                id="servo-show-target"
                checked={block.showTarget ?? true}
                onCheckedChange={(checked) =>
                  updateCurrent({ showTarget: checked } as ServoKnobBlock)
                }
              />
            </div>
          </>
        ...
        return (
          <div className="flex items-center justify-between">
            ...
            <Switch
              ...
              onCheckedChange={(checked) =>
                updateCurrent({ showPercent: checked } as MasterDimmerBlock)
              }
            />
          </div>
        );
    ...
    <div className="space-y-4">
      <div className="grid gap-3 md:grid-cols-2">
        ...
        <div className="space-y-1">
          ...
          <Select
            value={block.size ?? "md"}
            onValueChange={(value: CustomBlockSize) =>
              updateCurrent({ size: value })
            }
          >
            <SelectTrigger>
              <SelectValue />
      ...
      <div className="grid gap-3 md:grid-cols-4">
        {(
          ["col", "row", "width", "height"] as (keyof CustomBlockPosition)[]
        ).map((field) => (
          <div key={field} className="space-y-1">
            ...
            <Input
              ...
              onChange={(event) =>
                handlePositionChange(field, Number(event.target.value))
              }
            />
          </div>
        ))}
