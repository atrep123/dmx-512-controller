...
import { CopySimple, SquaresFour, TrashSimple } from "@phosphor-icons/react";
import {
  Fragment,
  useCallback,
  useRef,
...
type BindingInfo = { warning: string | null; preview: string | null };
...
const getBlockSummary = (block: CustomBlock) => {
  ...
  return position
    ? `${title} - c${position.col ?? 0} r${position.row ?? 0} w${
        position.width ?? 3
      } h${position.height ?? 1}`
    : title;
};

const getBlockSurfaceClass = (
  block: CustomBlock,
...
const withMeta = (
  ...
  const hasBindingDescription = Boolean(
    bindingInfo.warning || bindingInfo.preview
  );
  const position = block.position;
  const positionLabel = position
    ? `c${position.col ?? 0} r${position.row ?? 0}`
    : "bez pozice";
...
const BlockActionOverlay = ({
  ...
  return (
    <div className="pointer-events-none absolute inset-0 flex items-start justify-end opacity-0 transition group-hover:opacity-100">
      <div
        className={cn(
          "pointer-events-auto mt-2 mr-2 flex gap-1 rounded-full p-1 shadow",
          overlayClass
        )}
      >
        {actions.onDuplicate ? (
...
const createDefaultBlockRenderers = (
  ...
  "master-dimmer": (block, ctx) =>
    withMeta(
      ...
      appearanceStyles,
      getBlockSurfaceClass(
        block,
        palette,
        "from-slate-900/60 via-slate-900/20 to-slate-900/30"
      )
    ),
  "scene-button": (block, ctx) =>
    withMeta(
      ...
      getBlockSurfaceClass(
        block,
        palette,
        "from-rose-500/50 via-transparent to-rose-900/40"
      )
    ),
  "effect-toggle": (block, ctx) => {
    ...
    return withMeta(
      ...
      <div className="flex h-full flex-col justify-between">
        ...
        <div>
          <p className="text-lg font-semibold">
            {toggleBlock.title || "Effect"}
          </p>
          <p className="text-xs text-white/70">
            Režim: {toggleBlock.behavior ?? "toggle"}
          </p>
        </div>
        <div className="flex items-center gap-2 text-xs">
          <span className="inline-flex rounded-full bg-white/10 px-3 py-1">
            ON
          </span>
          <span className="text-white/60">/</span>
          <span className="inline-flex rounded-full border border-white/30 px-3 py-1">
            OFF
          </span>
        </div>
      </div>,
      ...
      getBlockSurfaceClass(
        block,
        palette,
        "from-amber-400/40 via-transparent to-amber-900/30"
      )
    );
  },
  "fixture-slider": (block, ctx) =>
    withMeta(
      ...
      getBlockSurfaceClass(
        block,
        palette,
        "from-blue-400/40 via-transparent to-blue-900/40"
      )
    ),
  "motor-pad": (block, ctx) =>
    withMeta(
      ...
      getBlockSurfaceClass(
        block,
        palette,
        "from-emerald-400/40 via-transparent to-emerald-900/40"
      )
    ),
  "servo-knob": (block, ctx) =>
    withMeta(
      ...
      getBlockSurfaceClass(
        block,
        palette,
        "from-purple-400/40 via-transparent to-purple-900/40"
      )
    ),
    ...
    return withMeta(
      ...
      <div className="flex h-full flex-col justify-between">
        <div>
          ...
          <p className="text-lg font-semibold">
            {noteBlock.title || "Instrukce"}
          </p>
        </div>
        <p className="line-clamp-4 text-sm text-white/80">
          {noteBlock.content || "Přidej instrukce nebo QR."}
        </p>
      </div>,
      ctx.bindingInfo ?? getBindingInfo(block),
      appearanceStyles,
      getBlockSurfaceClass(
        block,
        palette,
        "from-muted/40 via-transparent to-muted-foreground/30"
      )
    );
  },
});
...
const createFallbackRenderer =
  (appearanceStyles: AppearanceStyles): BlockRenderer =>
  (block, ctx) =>
    withMeta(
      block,
...
const getBindingInfo = (block: CustomBlock): BindingInfo => {
  switch (block.kind) {
    ...
    case "fixture-slider":
      ...
      return {
        warning: null,
        preview: `Fixture ${block.fixtureId} | kanál ${block.channelId}`,
      };
    case "motor-pad":
      return {
...
export function CustomLayoutRenderer({
  ...
  const appearanceStyles =
    appearancePresets[appearance] ?? appearancePresets.soft;
  const palette = { ...defaultBlockPalette, ...(blockPalette ?? {}) };
  const defaultRenderers = createDefaultBlockRenderers(
    appearanceStyles,
    palette
  );
  const mergedRenderers: BlockRendererMap = {
    ...defaultRenderers,
    ...renderers,
  };
  ...
  if (!blocks.length) {
    ...
    const fallbackEmpty = renderEmptyState?.() ?? (
      <div className="rounded-2xl border border-dashed border-muted-foreground/40 bg-muted/10 p-6 text-center text-sm text-muted-foreground">
        ...
        <p className="text-xs mt-1">
          Použij katalog bloků nebo import show snapshotu.
        </p>
      </div>
    );
  ...
  const gridGuides = showGridGuides
    ? {
        ...
        `,
        backgroundSize: `${Math.max(24, 96 / grid.columns)}px ${Math.max(
          24,
          grid.rowHeight * 48
        )}px`,
      }
    ...
    >
      {blocks.map((block, index) => {
        ...
        const width = Math.max(
          1,
          Math.min(block.position?.width ?? 3, grid.columns)
        );
        const height = Math.max(1, block.position?.height ?? 1);
        const isSelected =
          selectedBlockId === block.id ||
          Boolean(selectedBlockIds && selectedBlockIds.includes(block.id));
            ...
            <Fragment key={block.id}>
              {renderBlockWrapper({
                ...
                onSelect: onBlockSelect
                  ? (event) => onBlockSelect(block.id, event)
                  : undefined,
                onHoverChange: onBlockHoverChange
                  ? (isHovering) =>
                      handleHoverChange(isHovering ? block.id : null)
                  : undefined,
                children: content,
              })}
            </Fragment>
        ...
        return (
          <Fragment key={block.id}>
            <button
              ...
              data-highlighted={
                highlightedBlockId === block.id ? "true" : undefined
              }
              onClick={
                onBlockSelect
                  ? (event) => onBlockSelect(block.id, event)
            ...
            >
              ...
              <BlockActionOverlay
                block={block}
                actions={actions}
                overlayClass={appearanceStyles.overlay}
              />
            </button>
          </Fragment>
        );
